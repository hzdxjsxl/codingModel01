...
export class CRDTOperation {
  ...
  generateId() {
    return `op_${this.siteId || 'unknown'}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }

  static createInsert(charId, value, siteId, vectorClock) {
  ...
  static createDelete(charId, siteId, vectorClock) {
    return new CRDTOperation({
      type: 'crdt-delete',
      charId: charId,
      siteId: siteId,
  ...
  static fromJSON(json) {
    return new CRDTOperation(json);
  }

  toMessageFormat() {
    if (this.type === 'crdt-insert') {
      return {
        type: 'crdt-insert',
        id: this.charId,
        value: this.value,
...
export default { Operation, CRDTOperation };
