<template>
  <div class="container" :class="{ 'rollback-animation': showRollbackAnimation }">
    ...
    <div class="test-mode-section" v-if="!isProcessing && !isCompleted">
      ...
      <div v-if="testModeEnabled" class="test-mode-options">
        <label class="select-label">选择失败步骤（模拟补偿逻辑）：</label>
        <select 
          v-model="selectedFailStep"
          @change="onFailStepChange"
          class="step-select"
        >
          <option value="inventory">1. 检查库存</option>
          <option value="coupon">2. 作废优惠券</option>
          <option value="refund" selected>3. 微信退款（推荐测试补偿）</option>
          <option value="order">4. 更新订单状态</option>
        </select>
        <div class="test-hint">
          ⚠️ 选择"微信退款"步骤失败，将触发优惠券恢复的补偿逻辑
        </div>
      </div>
    </div>
...
import { refundApi, setTestMode } from './api/refundApi'
...
export default {
  ...
  setup() {
    ...
    const selectedFailStep = ref('refund')
    ...
    const onTestModeChange = () => {
      if (testModeEnabled.value) {
        setTestMode(true, selectedFailStep.value)
      } else {
        setTestMode(false, null)
      }
    }

    const onFailStepChange = () => {
      if (testModeEnabled.value) {
        setTestMode(true, selectedFailStep.value)
      }
    }
